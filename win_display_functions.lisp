...
(defun draw_world () 
    (progn 
        (loop for y below *level-height* do 
	        ...
	        (loop for x from (max 0 (- *player-x* *term-size*)) to (min (+ *player-x* *term-size*) 119) do
                    (if 
			(in_term_1 x y) 
	...
	(cls)
	(disp_messages)
	(when (every #'not *messages*) (terpri))
	...
	(loop for z below (length *tile-list*) do
	   ...
	   ;;(format t "~0t~a ~15t~a ~55t~c[34m~a ~c[0m~%" (get_status z) (nth z *tile-list*) #\ESC (get_command z) #\ESC)
	   (format t "~0t~a ~15t~a ~35t~a~%" (get_status z) (nth z *tile-list*) (get_command z))
	)
	(hp_warning)
	(fresh-line)
	(if (player-status_effects *player*) (princ (player-status_effects *player*)))
	(fresh-line)
	(princ *L2-pronoun*)
	(fresh-line)
	(setf *tile-list* nil)))
...
(defun get_tile (x y) 
    ...
    (cond ((is_player_position x y) *player-tile*) 
	  ...
	  ((or (t_line x y) (and *fuego* (is_fire x y))) "*")
